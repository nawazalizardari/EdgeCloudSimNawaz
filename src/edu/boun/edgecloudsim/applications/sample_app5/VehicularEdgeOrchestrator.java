...
//	public static final int CLOUD_DATACENTER_VIA_GSM = 1;
	public static final int CLOUD_DATACENTER_VIA_5G = 1;
	...
	public void initialize() {
		...
		// assume the first app has the lowest and the last app has the highest task
		// length value
	...
	public int getDeviceToOffload(Task task) {
		 ...
		 * networkModel.estimateUploadDelay(NETWORK_DELAY_TYPES.GSM_DELAY, task); double
		 * gsmDownloadDelay =
		 * networkModel.estimateDownloadDelay(NETWORK_DELAY_TYPES.GSM_DELAY, task);
		 */

		double fiveGUploadDelay = networkModel.estimateUploadDelay(NETWORK_DELAY_TYPES.FIVEG_DELAY, task);
		double fiveGDownloadDelay = networkModel.estimateDownloadDelay(NETWORK_DELAY_TYPES.FIVEG_DELAY, task);
		...
		int options[] = { EDGE_DATACENTER, CLOUD_DATACENTER_VIA_RSU, CLOUD_DATACENTER_VIA_5G };
		...
		if (policy.startsWith("AI_") || policy.equals("MAB") || policy.equals("GAME_THEORY")) {
			...
			// TODO: Add MAX_FiveG_DELAY IN WekaWrapper
			if (fiveGUploadDelay == 0)
				fiveGUploadDelay = WekaWrapper.MAX_GSM_DELAY;

			if (fiveGDownloadDelay == 0)
				fiveGDownloadDelay = WekaWrapper.MAX_GSM_DELAY;
		...
		if (policy.equals("AI_BASED")) {
			...
			boolean predictedResultForEdge = weka.handleClassification(EDGE_DATACENTER,
					new double[] { trainerLogger.getOffloadStat(EDGE_DATACENTER - 1), task.getCloudletLength(),
							wlanUploadDelay, wlanDownloadDelay, avgEdgeUtilization });

			boolean predictedResultForCloudViaRSU = weka.handleClassification(CLOUD_DATACENTER_VIA_RSU, new double[] {
					trainerLogger.getOffloadStat(CLOUD_DATACENTER_VIA_RSU - 1), wanUploadDelay, wanDownloadDelay });

//			boolean predictedResultForCloudViaGSM = weka.handleClassification(CLOUD_DATACENTER_VIA_GSM, new double[] { trainerLogger.getOffloadStat(CLOUD_DATACENTER_VIA_GSM - 1), gsmUploadDelay, gsmDownloadDelay });

			boolean predictedResultForCloudViaFiveG = weka.handleClassification(CLOUD_DATACENTER_VIA_5G, new double[] {
					trainerLogger.getOffloadStat(CLOUD_DATACENTER_VIA_5G - 1), fiveGUploadDelay, fiveGDownloadDelay });
...
//				predictedServiceTimeForCloudViaGSM = weka.handleRegression(CLOUD_DATACENTER_VIA_GSM, new double[] { task.getCloudletLength(), fiveGUploadDelay, fiveGDownloadDelay });

			if (predictedResultForCloudViaFiveG)
				predictedServiceTimeForCloudViaGSM = weka.handleRegression(CLOUD_DATACENTER_VIA_5G,
						new double[] { task.getCloudletLength(), fiveGUploadDelay, fiveGDownloadDelay });

			if (!predictedResultForEdge && !predictedResultForCloudViaRSU && !predictedResultForCloudViaFiveG) {
			...
			} else if (predictedServiceTimeForEdge <= Math.min(predictedServiceTimeForCloudViaRSU,
					predictedServiceTimeForCloudViaGSM))
				result = EDGE_DATACENTER;
			else if (predictedServiceTimeForCloudViaRSU <= Math.min(predictedServiceTimeForEdge,
					predictedServiceTimeForCloudViaGSM))
				result = CLOUD_DATACENTER_VIA_RSU;
			else if (predictedServiceTimeForCloudViaGSM <= Math.min(predictedServiceTimeForEdge,
					predictedServiceTimeForCloudViaRSU))
				result = CLOUD_DATACENTER_VIA_5G;
			else {
...
//					trainerLogger.addStat(task.getCloudletId(), result, wanUploadDelay, wanDownloadDelay, gsmUploadDelay, gsmDownloadDelay, wlanUploadDelay, wlanDownloadDelay);
					trainerLogger.addStat(task.getCloudletId(), result, wanUploadDelay, wanDownloadDelay,
							fiveGUploadDelay, fiveGDownloadDelay, wlanUploadDelay, wlanDownloadDelay);
			...
			if (!resultFound) {
				SimLogger.printLine(
						"Unexpected probability calculation for AI based orchestrator! Terminating simulation...");
				System.exit(1);
			}
			...
			if (!resultFound) {
				SimLogger.printLine(
						"Unexpected probability calculation for random orchestrator! Terminating simulation...");
				System.exit(1);
			}
...
//				double[] expectedDelays = { wlanUploadDelay + wlanDownloadDelay + expectedProcessingDealyOnEdge, wanUploadDelay + wanDownloadDelay + expectedProcessingDealyOnCloud, gsmUploadDelay + gsmDownloadDelay + expectedProcessingDealyOnCloud };
				double[] expectedDelays = { wlanUploadDelay + wlanDownloadDelay + expectedProcessingDealyOnEdge,
						wanUploadDelay + wanDownloadDelay + expectedProcessingDealyOnCloud,
						fiveGUploadDelay + fiveGDownloadDelay + expectedProcessingDealyOnCloud };
			...
			double expectedEdgeDelay = expectedProcessingDealyOnEdge + wlanUploadDelay + wlanDownloadDelay;
...
//			double expectedCloudDelay = expectedProcessingDealyOnCloud + (isGsmFaster ? gsmUploadDelay : wanUploadDelay) + (isGsmFaster ? gsmDownloadDelay : wanDownloadDelay);
			double expectedCloudDelay = expectedProcessingDealyOnCloud
					+ (isGsmFaster ? fiveGUploadDelay : wanUploadDelay)
					+ (isGsmFaster ? fiveGDownloadDelay : wanDownloadDelay);
			...
			double Pi = GTH.getPi(task.getMobileDeviceId(), taskArrivalRate, expectedEdgeDelay, expectedCloudDelay,
					maxDelay);
			...
			if (Pi < randomNumber)
				...
				result = (isGsmFaster ? CLOUD_DATACENTER_VIA_5G : CLOUD_DATACENTER_VIA_RSU);
			...
			if (CloudSim.clock() > SimSettings.getInstance().getWarmUpPeriod()) {
				/*
				 * failureRate_i = 100 * numOfFailedTask / (numOfFailedTask +
				 * numOfSuccessfulTask)
				 */
				double failureRates[] = { statisticLogger.getFailureRate(options[0]),
						statisticLogger.getFailureRate(options[1]), statisticLogger.getFailureRate(options[2]) };

				double serviceTimes[] = { statisticLogger.getServiceTime(options[0]),
						statisticLogger.getServiceTime(options[1]), statisticLogger.getServiceTime(options[2]) };
			...
			if (!resultFound) {
				SimLogger.printLine(
						"Unexpected probability calculation for predictive orchestrator! Terminating simulation...");
				System.exit(1);
			}
		} else {
	...
	public Vm getVmToOffload(Task task, int deviceId) {
		...
		if (deviceId == CLOUD_DATACENTER_VIA_5G || deviceId == CLOUD_DATACENTER_VIA_RSU) {
			...
			Vehicle v = new Vehicle(task.getMobileDeviceId(), loc.getXPos(), loc.getYPos(), -1);
//			VehicularMobilityModel m = (VehicularMobilityModel) SimManager.getInstance().getMobilityModel();
...
//			int hostIndex = (edgeVmCounter / vmPerHost) % numOfEdgeHosts;
	...
	public void startEntity() {
		if (policy.equals("PREDICTIVE")) {
			schedule(getId(), SimSettings.CLIENT_ACTIVITY_START_TIME
					+ OrchestratorStatisticLogger.PREDICTION_WINDOW_UPDATE_INTERVAL, UPDATE_PREDICTION_WINDOW);
		}
	}
	...
	public void processEvent(SimEvent ev) {
		...
		case UPDATE_PREDICTION_WINDOW: {
			...
			schedule(getId(), OrchestratorStatisticLogger.PREDICTION_WINDOW_UPDATE_INTERVAL, UPDATE_PREDICTION_WINDOW);
			break;
		}
		default:
